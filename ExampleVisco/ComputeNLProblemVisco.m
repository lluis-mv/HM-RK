...
function [X, GPInfo, normResidual, ThisInfo] = ComputeNLProblemVisco(Nodes, Elements, CP, dt, nSteps, ElementType, RKMethod, AlphaStabM, Iterate)
...
for el = 1:nElements
    for gp = 1:size(GPInfo,2)
        GPInfo(el,gp).DPrev = rand(6,6);
        GPInfo(el,gp).eVPPrev = rand(6,6);
    end
end
...
for loadStep = 1:nSteps
    ...
    for i = 1:length(b)
        ...
        ss = 0;
        while (true)
            GPInfo = EvaluateConstitutiveLawVisco(GPInfo, CP,  dt, k, a, b,  i, true);
        
            % Create again C with the appropriate ElastoPlastic stiffness matrix
            [C, K ] = EnsambleMatrices(Nodes, Elements, GPInfo, CP, ElementType, RKMethod,  dt, false, AlphaStabM);
            
            [C, K,  ~, ~, ~] = ApplyBoundaryConditions(Nodes, Elements, GPInfo, C, K);
            fVisco = ComputeViscousForces(Nodes, Elements, GPInfo, CP, ElementType, X);
            fVisco(nDirichlet) = 0;
            k(:,i) = C\(K*XStep + f + fVisco + uDirichlet);
            if ( loadStep == 1)
                k(:,i) = k(:,i) + (1/dt)* (C\fini);
            end
%             norm(k(:,i)-kPrev)
            ...
            if ( Iterate == false && loadStep > 1)
                ...
                GPInfo = EvaluateConstitutiveLawVisco(GPInfo, CP, dt, kConst, a, b, i, false);
            else
                GPInfo = EvaluateConstitutiveLawVisco(GPInfo, CP, dt, k, a, b, i, false);
            end
    end
    
    GPInfo = EvaluateConstitutiveLawVisco(GPInfo, CP, dt, k, a, b);
...
function f = ComputeViscousForces(Nodes, Elements, GPInfo, CP, ElementType, X)
...
for el = 1:nElements
    ...
    for gp = 1:size(GPInfo, 2)
        ...
        ThisStress = GPInfo(el,gp).D6*GPInfo(el,gp).eVP;
        f(index) = f(index) + GPInfo(el,gp).B'* ThisStress([1,2,4])*GPInfo(el,gp).Weight;
