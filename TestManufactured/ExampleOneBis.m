...
for Elem = [1,2,3,4]
    ...
    for MyNumber = [10]
        ...
        for eSize = ESIZE
            ...
            RKMethod = 1;
            [U,GPInfo] = ComputeLinearProblem(Nodes, Elements, CP, dt, nSteps, ElementType, RKMethod, 0);
            
            %PlotNodal(Nodes, Elements, 1000*U(3:3:end), true);
            
            
            [Xa] = ComputeAnalyticalSolution(Nodes, Elements, ElementType, t, CP, GPInfo, U);
            [L2(i), L2U(i), LInf(i), LInfU(i)] = ComputeErrorNorms2(U, Xa, Nodes, Elements, GPInfo);
            
            
            %figure(2105); clf; PlotNodal(Nodes, Elements, U(2:3:end)); colorbar; hold off;
            %figure(2106); clf; PlotNodal(Nodes, Elements, Xa(2:3:end)); colorbar; hold off;
            %figure(2107); clf; PlotNodal(Nodes, Elements, U(2:3:end)-Xa(2:3:end)); colorbar; hold off;
            %figure(2108); clf; PlotNodal(Nodes, Elements, U(2:3:end)-Xa(2:3:end), true); colorbar; hold off;
        ...
        figure(50)
        name = [ElementType(1:2)];
        merda = loglog(esizeAxis, L2U, [thisColor, '*-.']);
