...
for nNodes = [100,500,1000,5000,10000,50000]
    fig = 1;
    for j = [0,0.1,0.2,0.5]

        figure(fig)
        [solution(i).t, solution(i).pw] = Aux(nNodes, j);
        ...
        title(num2str(j))
        ylim([-0.1, 1.1])
        legend('100','500','1000','5000','10000','50000','location','best')
        pause(0.01)
        fig = fig+1;
    end
    i = i+1;
...
function [timePointOfInterest, waterPressurePointOfInterest] = Aux(nNodes, alphaValue)
...
if ( PPP == false && alphaValue > 0 )
%     AlphaStab = h^2/M*(rho_mixture-rho_water)/rho_water * porosity/(porosity-1) * alphaValue;
    AlphaStab = h^2/M*alphaValue;
    tau = (rho_mixture-rho_water)/rho_water * porosity/(porosity-1) ;
    DampingMatrix([3,6],[3,6]) = DampingMatrix([3,6],[3,6])  + AlphaStab*(tau-1) * int( DN_DX*DN_DX', x, 0, h);
    MassMatrix([3,6],[3,6]) = MassMatrix([3,6],[3,6]) + AlphaStab*tau/k* int( N*DN_DX', x, 0, h);
end
...
tt = 0;
invSystemMatrix = inv(SystemMatrix);
