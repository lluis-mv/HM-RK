...
def plot_msh(file, tipo, mostrar_nodos=False, mostrar_num_nodo=False, 
    ...
    this_file = os.path.join( problem_path, "NodesQ.m")
    this_file = open(this_file, "w")
    this_file.write( " Nodes = [ ")
    for ii in range(0, int( float(xnod.size)/float(2)) ):
        this_file.write( str(xnod[ii,0]) + " , "+  str(xnod[ii,1]) +  " ;  \n")
    this_file.write( " ]; \n")    
    
    
    
    this_file = os.path.join( problem_path, "ElementsQ.m")
    this_file = open(this_file, "w")
    this_file.write( " Elements = [ ")
    print(LaG_mat)
    for ii in range(0, int( float(LaG_mat.size)/float(9)) ):
        line = '';
        for jj in range(1, 9):
            line = line + str(LaG_mat[ii, jj]+1) + " ,"
        line = line[:-1] + " ; \n"
